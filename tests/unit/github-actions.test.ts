...
describe('GitHub Actions Formatter', () => {
  ...
  describe('formatAnnotation', () => {
    test('formats basic annotation correctly', () => {
      ...
      const expected = '::warning file=src/utils.ts,line=42,title=Function signature changed::Function add parameter count changed from 2 to 3';
    ...
    test('formats annotation with endLine', () => {
      ...
      const expected = '::error file=src/components/Button.tsx,line=15,endLine=20,title=Component structure changed::JSX structure modified, may affect rendering';
      expect(formatAnnotation(annotation)).toBe(expected);
    });
    ...
    test('formats notice annotation', () => {
      ...
      const expected = '::notice file=src/types.ts,line=8,title=Type definition changed::Interface property added: newField';
      expect(formatAnnotation(annotation)).toBe(expected);
    });
